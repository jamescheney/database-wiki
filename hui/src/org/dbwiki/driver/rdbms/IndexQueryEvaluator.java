 ...
 *
 */
public class IndexQueryEvaluator extends DatabaseReader {
	/*
	 * Public Methods
	 */
	 
	
	public static QueryResultSet evaluate(Connection con, RDBMSDatabase database, WikiPathQueryStatement query) throws java.sql.SQLException, org.dbwiki.exception.WikiException, IOException {
		long startevalute = System.nanoTime();
		
		Vector<NodeIdentifier> entries = null;
		VectorQueryResultSet rs = new VectorQueryResultSet();	
	 
	if (query.firstElement().hasAxis()){
		if (query.firstElement().axis().equals("child")){
	if (query.firstElement().hasCondition()) {
	...
	}else if (query.firstElement().hasCondition()) {
		if (query.firstElement().condition().isIndexCondition()) {
			entries = new Vector<NodeIdentifier>();
			NodeIdentifier nid = findEntryByIndex(con, database, ((WikiPathIndexCondition)query.firstElement().condition()).index());
		...
		for (int iEntry = 0; iEntry < entries.size(); iEntry++) {
			...
			RDBMSDatabaseGroupNode entry = (RDBMSDatabaseGroupNode)get(con, database, entries.get(iEntry)); //get context 
	...
	private static void eval(DatabaseElementNode node, WikiPathQueryStatement query, int pos, VectorQueryResultSet rs) throws org.dbwiki.exception.WikiException,java.sql.SQLException, org.dbwiki.exception.WikiException  {
		WikiPathComponent component = query.get(pos);
		boolean isValid = true;

	 if(component.hasAxis()) {
		 if(component.axis().equals("child")){
	 ...
	 }else if (component.hasCondition()) {
			if (component.condition().isValueCondition()) {
				isValid = false;
				WikiPathValueCondition valueCond = (WikiPathValueCondition)component.condition();
				DatabaseElementList nodes = ((DatabaseGroupNode)node).find(valueCond.schema());
