...
public class PartialIndexPrinter extends IndexContentPrinter {
	...
	private static final int entriesPerPage = 50;
	...
	public void listContent(DatabaseIdentifier databaseIdentifier, ContentIterator iterator, WikiRequest request, HtmlLinePrinter body) throws org.dbwiki.exception.WikiException {
		int maxOutputCount = entriesPerPage;
		int outputCount = 0;
		int readCount = 0;
		int startIndex = 1;
		...
		while ((entry = iterator.next()) != null) {
			...
			if (readCount >= startIndex) {
				body.add("<li>" + this.getLink(databaseIdentifier, entry, versionParameter) + "</li>");
				outputCount++;
			}
		...
		if (outputCount > 0) {
			...
			if (startIndex > 1) {
				navigationLine = "<a CLASS=\"" + CSS.CSSIndexNavbar + "\" HREF=\"" + databaseIdentifier.databaseHomepage() + "?" + RequestParameter.ParameterIndexPosition + "=" + Math.max(1, (startIndex - entriesPerPage));
