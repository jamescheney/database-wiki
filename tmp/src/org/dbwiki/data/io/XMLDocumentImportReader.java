...
import org.dbwiki.data.database.DataImportReader;
import org.dbwiki.data.database.ImportHandler;
...
public class XMLDocumentImportReader implements DataImportReader {
	/*
	 * Private Variables
	 */
	...
	public XMLDocumentImportReader(String sourceURL, DatabaseSchema schema, String rootPath, User user, boolean validating, boolean xmlSchema) throws org.dbwiki.exception.WikiException {
		...
		_inputStream = null;
		try {
			if (sourceURL.endsWith(".gz")) {
				_inputStream = new GZIPInputStream(new URL(sourceURL).openStream());
			} else {
				_inputStream = new URL(sourceURL).openStream();
			}
	...
	public void start() throws org.dbwiki.exception.WikiException {
		...
		// jcheney: documentHandler transforms InputHandler events ImportHandler, agglomerating char blocks
		...
		try {
			new SAXDocumentParser().parse(_inputStream, _validating, _xmlSchema, callbackHandler);
			_inputStream.close();
