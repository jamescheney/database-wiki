...
public class RDBMSDatabase implements Database, DatabaseConstants {
	...
	private static final int MAX_NUMBER_OF_NODES=100000;
	...
	public void exportHistory(ResourceIdentifier identifier,ExportHistoryWriter out) throws IOException, WikiException {	
			
		DatabaseNode node=get(identifier);
		out.setMainNode(node);
		out.startDatabase(_identifier.linkPrefix());
		SizeOptimizer opt = new SizeOptimizer(node,MAX_NUMBER_OF_NODES);	
		exportHistoryNode(node,out,opt.getFirstHiddenLayer());		
		out.bottomUp(node, this.name());
		out.endDatabase();
		out.outputLayoutedGraph();
	}

	

	private void exportHistoryNode(DatabaseNode node, ExportHistoryWriter out, int levelsToRender) throws WikiException {
		levelsToRender--;
		DatabaseElementNode element=null;
			if (node.isElement()) {
				...
				if (element.isAttribute()) {
					...
					out.writeAttributeNode(attribute, levelsToRender);
				} else {
					DatabaseGroupNode group = (DatabaseGroupNode)element;
					out.writeGroupNode(group,levelsToRender);
	
					if (levelsToRender!=0){
						for (int iChild = 0; iChild < group.children().size(); iChild++) {
							exportHistoryNode(group.children().get(iChild),out, levelsToRender);
						}
					}
				}
			} else {
				out.writeLeafNode((DatabaseTextNode)node);			
			}		
	}
	...
	public String getValue(ResourceIdentifier identifier) throws org.dbwiki.exception.WikiException {
		...
		try {
			Connection con = _connector.getConnection();
			value = DatabaseReader.getValue(con, this, (NodeIdentifier)identifier);
			con.close();
		} catch (java.sql.SQLException sqlException) {
			throw new WikiFatalException(sqlException);
		}
