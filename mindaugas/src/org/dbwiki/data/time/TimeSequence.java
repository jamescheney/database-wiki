...
public class TimeSequence {
	...
	public String toString(){
		String ret="";
		TimeInterval[] tInterval=this.intervals();
		for (int i=0; i<tInterval.length; i++){
			int start=tInterval[i].start();
			int end=tInterval[i].end();
			if(start!=end)
				ret+=start+"-";
			
			if ((i+1)<tInterval.length)
				ret+=end+", ";
			else{
				if (this.isCurrent())
					ret+="cur.";
	...
	public String printActiveIntervals(VersionIndex vi) throws WikiException {
		String ret="";
		TimeInterval[] tInterval=this.intervals();
		for (int i=0; i<tInterval.length; i++){
			if (i>0)
			...
			ret+="  "+vi.getByNumber(start).name().trim()+"-";
			if (end==-1)
				ret+="Now";
			else
				ret+=vi.getByNumber(end+1).name().trim();
			
			if (vi.getByNumber(start).provenance().user()!=null){
				String user=vi.getByNumber(start).provenance().user().fullName();
				ret+="  "+ExportHistoryWriter.formForDot(user);
			}
			else
				ret+="  Unknown";

			ret+="  "+vi.getByNumber(start).provenance().name();
	...
	public ArrayList<Integer> getFirstValues() {
		...
		for (int i=0; i<this.intervals().length; i++){
			firstValues.add(this.intervals()[i].start());				
		}
		return firstValues;
	}
