...
public abstract class MenuPrinter implements HtmlContentPrinter {
	...
	public void print(HtmlLinePrinter printer) throws org.dbwiki.exception.WikiException {
		...
		if(_request.type().isHistoryTree() || _request.type().isStructureTree()){
			this.printVisualisationMenu(printer);
		}
		else{
	...
	private void printVisualisationMenu(HtmlLinePrinter printer) {
		if (_request.type().isHistoryTree())
			printer.add("\t\t<a id=\"t_history_tree\" class=\"" + CSS.CSSMenu + "\" href="+_request.wri().getURL() + 
					"?" + RequestParameter.ParameterHistoryTree+"><b>History tree</b></a>");
		else
			printer.add("\t\t<a id=\"t_history_tree\" class=\"" + CSS.CSSMenu + "\" href="+_request.wri().getURL() + 
					"?" + RequestParameter.ParameterHistoryTree+">History tree</a>");
		
		if (_request.type().isStructureTree())
			printer.add("\t\t<a id=\"t_structure_tree\" class=\"" + CSS.CSSMenu + "\" href="+_request.wri().getURL() + 
					"?" + RequestParameter.ParameterStructureTree+"><b>Structure tree</b></a>");
		else
			printer.add("\t\t<a id=\"t_structure_tree\" class=\"" + CSS.CSSMenu + "\" href="+_request.wri().getURL() + 
					"?" + RequestParameter.ParameterStructureTree+">Structure tree</a>");
