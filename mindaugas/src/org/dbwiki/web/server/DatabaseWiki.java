...
public class DatabaseWiki implements HttpHandler, Comparable<DatabaseWiki> {
	...
	private void respondToValueRequest(WikiDataRequest request) throws WikiException {
 
		
		String value=database().getValue(request.wri().resourceIdentifier());
		try {
			_server.sendTXT(request.exchange(), new ByteArrayInputStream(value.getBytes("UTF-8")));
	...
	private void respondToExportDotRequest(WikiDataRequest request) throws org.dbwiki.exception.WikiException {
		try {
			long start = System.currentTimeMillis();
			StringWriter buf = new StringWriter();
			BufferedWriter out = new BufferedWriter(buf);
			ExportHistoryWriter exportHistoryWriter= new ExportHistoryWriter(request, out, database().versionIndex());
			database().exportHistory(request.wri().resourceIdentifier(),exportHistoryWriter);
			out.close();
			String output = buf.toString();
			_server.sendTXT(request.exchange(), new ByteArrayInputStream(output.getBytes("UTF-8")));
	...
	private void respondToDataRequest(WikiDataRequest request) throws java.io.IOException, org.dbwiki.exception.WikiException {
		...
		if (request.type().isAction()) {
			...
			return;
		} else if (request.type().isExportHistoryDot() || request.type().isExportStructureDot()) {
			this.respondToExportDotRequest(request);
			return;	
		} else if (request.type().isValue()) {
			this.respondToValueRequest(request);
			return;	
		...
		if (page == null) {
			...
			} else if (request.type().isHistoryTree() || request.type().isStructureTree()) {
