...
import org.dbwiki.driver.rdbms.ProvenanceGraphSQLQueries;
...
import org.dbwiki.web.request.URLComponent;
...
public class DatabaseWiki implements HttpHandler, Comparable<DatabaseWiki> {
	...
	public static final String AnnotationRequestPrefix = "annotation"; //New constant is added for Annotation
	public static final String AnnotationStructureRequestPrefix = "annotationStructure"; //New constant is added for Annotation visualisation
	public static final String ProvenanceStructureRequestPrefix = "provenanceStructure"; //New constant is added for Provenance visualisation
	public static final String ProvenanceComparisonRequestPrefix = "provenanceComparison"; //New constant is added for comparison of 2 provenance records
	public static final String ProvenanceTreeRequestPrefix = "provenanceTree"; //New constant is added for Provenance Tree
	...
	public void handle(HttpExchange exchange) throws java.io.IOException {
		try {
			...
			if (pos != -1) {
				...
				/* This if case is added to identify the URL for annotation request*/
				if(url.isAnnotationRequest()) {
					URLComponent nodeID = url.get(0);							
					respondToAnnotationRequest(new WikiDataRequest(this, url),nodeID.decodedText());
				}
				/* This if case is added to identify the URL for annotation visualisation request*/
				else if(url.isAnnotationStructureRequest()) {
					respondToAnnotationStructureRequest(new WikiDataRequest(this, url));
				}
				/* This if case is added to identify the URL for provenance visualisation and for comparing 2 provenance records request*/
				else if(url.isProvenanceStructureRequest() || url.isProvenanceComparisonRequest()) {
					respondToProvenanceStructureRequest(new WikiDataRequest(this, url));
				}
				/* This if case is added to identify the URL for displaying provenance in tree structure request*/
				else if(url.isProvenanceTreeRequest()) {
					respondToProvenanceTreeRequest(new WikiDataRequest(this, url));
				}
				else if (url.isDataRequest()) {
	...
	/* It processes the visualisation of Annotation request.
	 * It first checks its request type for annotation i.e. charts or list 
	 * and either processes it or passes it to the corresponding method. 
	 * Also generates response page to send it to the HtmlSender*/

	private void respondToAnnotationStructureRequest(WikiDataRequest request) throws WikiException, SQLException, IOException {
		// TODO Auto-generated method stub
		RequestParameterList parameterList= request.parameters();
		RequestParameter displayStr = parameterList.get("displayStr");
		RequestParameter username = parameterList.get("username");
		RequestParameter startDate = parameterList.get("startDate");
		RequestParameter endDate = parameterList.get("endDate");
		String result = "";
		
		if(displayStr.value().equalsIgnoreCase("charts"))
		{
			result = database().AnnotationCharts(startDate.value(), endDate.value());
			HtmlSender.sendSQLData(result, request.exchange());
		}
		else if(displayStr.value().equalsIgnoreCase("list"))
		{
			AnnotationList annotations = database().AnnotationTableByUser(username.value(), startDate.value(), endDate.value());
			HtmlPage page = new HtmlPage();
			
			if(annotations.size() != 0)
			{
				page.add("<table id=\"AnnotationTableByUser\">");
				page.add("<tr>");
				page.add("<th>Comment</th>");
				page.add("<th>User</th>");
				page.add("<th>Date</th>");
				page.add("</tr>");
				...
				for (int iAnnotation = 0; iAnnotation < annotations.size(); iAnnotation++) {
					page.add("<tr>");
					page.add("<td>");
					page.add(annotations.get(iAnnotation).text());
					page.add("</td>");
					page.add("<td>");
					if (annotations.get(iAnnotation).user() != null) {
						page.add(annotations.get(iAnnotation).user().fullName());
					} else {
						page.add(User.UnknownUserName);
					}
					page.add("</td>");
					page.add("<td>");
					page.add(annotations.get(iAnnotation).date());
					page.add("</td>");
					page.add("</tr>");
				}
				page.add("</table>");
	...
	/* It processes the request of displaying provenance tree structure.
	 * Currently, this function is passing the static values
	 * Future work is required to extract the data from database using node id and version id 
	 * send data in JSON format*/
	private void respondToProvenanceTreeRequest(WikiDataRequest request) throws JSONException, IOException {
		RequestParameterList parameterList= request.parameters();
		RequestParameter versionID = parameterList.get("versionId");
		VersionIndex versions = request.versionIndex();
		int versionId = new Integer(versionID.value());
		
		String temp = "{\"id\":1,\"content\":\"User 1\",\"children\": [{\"id\":2,\"content\":\"User 2\",\"children\": [{\"id\":11,\"content\":\"User 3\"},{\"id\":12,\"content\":\"User 4\",\"children\": [{\"id\":15,\"content\":\"User 5\",},{\"id\":16,\"content\":\"User 6\"}]},{\"id\":12,\"content\":\"User 7\"}]},{\"id\":3,\"content\":\"User 9\"}]}";
	
		JSONObject obj=new JSONObject(temp);
		HtmlSender.sendJSON(obj, request.exchange());
		
	}
	
	/* It processes the visualisation of provenance request.
	 * It first checks its request type for provenance i.e. charts, list or comparison 
	 * and either processes it or passes it to the corresponding method. 
	 * Also generates response page to send it to the HtmlSender
	 * The implementation of comparison of 2 provenance records working for the child nodes
	 * Future work is required to implement it for all nodes*/
	private void respondToProvenanceStructureRequest(WikiDataRequest request) throws WikiException, SQLException, IOException {
		// TODO Auto-generated method stub
		
		RequestParameterList parameterList= request.parameters();
		RequestParameter displayStr = parameterList.get("displayStr");		
		String result = "";
		VersionIndex versionList;
		if(displayStr.value().equalsIgnoreCase("charts"))
		{
			RequestParameter username = parameterList.get("username");
			RequestParameter startDate = parameterList.get("startDate");
			RequestParameter endDate = parameterList.get("endDate");
			result = database().ProvenanceCharts(startDate.value(), endDate.value());
			HtmlSender.sendSQLData(result, request.exchange());
		}
		else if(displayStr.value().equalsIgnoreCase("list"))
		{
			...
			RequestParameter startDate = parameterList.get("startDate");
			RequestParameter endDate = parameterList.get("endDate");
			versionList = database().ProvenanceList(username.value(), startDate.value(), endDate.value());
			HtmlPage page = new HtmlPage();
			page.add("<table class=\"ProvenanceList\">");
			page.add("<tr>");
			page.add("<th>Version</th>");
			page.add("<th>User</th>");
			page.add("<th>Action</th>");
			page.add("</tr>");
			...
			for(int i=0; i< versionList.size(); i++)
			{
				...
				page.add("<tr>");
				page.add("<td>");
				page.add(version.name());
				page.add("</td>");
				page.add("<td>");
				if (version.provenance().user() != null) {
					page.add(version.provenance().user().fullName());
				} else {
					page.add(User.UnknownUserName);
				}
				page.add("</td>");
				...
				if (version.provenance().isCopy()) {
					page.add(version.provenance().name());
		...
		else if(displayStr.value().equalsIgnoreCase("comparison"))
		{
			RequestParameter version1 = parameterList.get("version1");
			RequestParameter version2 = parameterList.get("version2");
		/*	RequestParameterVersion _versionParameter = RequestParameter.versionParameter(request.parameters().get("version1"));
			DatabaseNode dbNode = database().get(request.wri().resourceIdentifier());
			System.out.println(dbNode+" dbNode@@@@");
			DatabaseGroupNode node = (DatabaseGroupNode)dbNode;
			System.out.println(node+" DatabaseGroupNode@@@@");
			DataNodePrinter printer = new DataNodePrinter(_database.identifier(), _layouter);
			System.out.println(printer+"@@@@ printer");
			HtmlLinePrinter body = new HtmlLinePrinter();
			System.out.println(body+"@@@@ body");
			printer.printGroupNode(node, _versionParameter, body);
			...
			for (int i = 0; i < children.size(); i++) {
				...
				HtmlPage lines = printer.getLinesForNodeList(children.get(i), _versionParameter);
				System.out.println("lines::" + lines);
			}
			
			
			DatabaseNode node = (DatabaseNode)(database().get(request.wri().resourceIdentifier()));
			Vector<Version> versionVec =  request.versionIndex().getNodeChanges(node);
			Version ver = versionVec.get(5);
			DatabaseNode targetNode = request.node().find(ver.provenance().identifier());
			DatabaseGroupNode gNode = (DatabaseGroupNode)targetNode ;
			...
			System.out.println("node::" + ((NodeIdentifier)request.node().identifier()).nodeID()); //delete
			DatabaseElementNode node = (DatabaseElementNode)(database().get(request.wri().resourceIdentifier()));
			
			VersionIndex versions = request.versionIndex();
			int v1 = new Integer(version1.value());
			int v2 = new Integer(version2.value());
			String r1 = database().versionComparison("287720");
			String r2 = database().versionComparison("287723");
			
			HtmlPage page = new HtmlPage();
			page.add("<table id=\"ComparisonTable\">");
			page.add("<tr>");
			page.add("<th>");
			page.add("Old Version");
			page.add("</th>");
			page.add("<th>");
			page.add("New Version");
			page.add("</th>");
			page.add("</tr>");
			page.add("<tr>");
			page.add("<td id=\"one\">");
			...
			page.add("</tr>");
			page.add("<tr>");
			page.add("<td colspan=\"2\">");
			page.add("<div class=\"diff\"></br>Difference......</br>(<ins>added part</ins> - <del>deleted part</del>)</div>");
			page.add("</td>");
			page.add("</tr>");
			page.add("</table>");
	...
	/* It processes the Annotation request.
	 * It first detects the request type for annotation i.e. show, insert or delete 
	 * and either processes it or passes it to the corresponding method. */
	private void respondToAnnotationRequest(WikiDataRequest request, String nodeID) throws WikiException, IOException {	
		
		// It checks the user login
		if ((request.user() == null) && (_authenticator.getAuthenticationMode() != WikiAuthenticator.AuthenticateNever)) {
			...
			try {
				HtmlPage page = new HtmlPage();
				String loginRedirectLink = WikiServer.SpecialFolderLogin + "?" + RequestParameter.ParameterResource + "=" + URLEncoder.encode(request.getRequestURI().toASCIIString(), "UTF-8");
				page.add("<p CLASS=\"" + CSS.CSSLogin + "\"><a CLASS=\"" + CSS.CSSLogin + "\" HREF=\"" + loginRedirectLink + "\">Login</a></p>");
				HtmlSender.send(page, request.exchange());
		...
		RequestParameterList parameterList= request.parameters();
		RequestParameter Actionparameter=parameterList.get("action");
		
		if(Actionparameter.value().equalsIgnoreCase("show")) //This if case is used for show annotation request
		{
			showAnnotation(request,nodeID);
			
		} 
		else if(Actionparameter.value().equalsIgnoreCase("insert")) //This if case is used for insert annotation request
		{
			RequestParameter annotateParameter = parameterList.get("annotate");
						
			if (annotateParameter.hasValue()) {
				if (!annotateParameter.value().trim().equals("")) {
					database().annotate(request.wri().resourceIdentifier(), new Annotation(annotateParameter.value(), new SimpleDateFormat("d MMM yyyy HH:mm:ss").format(new Date()), request.user()));
					showAnnotation(request,nodeID);
				}
			}
		} 
		else if(Actionparameter.value().equalsIgnoreCase("delete")) //This if case is used for delete annotation request
		{
			RequestParameter a_id = parameterList.get("annotation_id");
			database().deleteAnnotation(request.wri().resourceIdentifier(),a_id.value());
	...
	/* This method creates the list of all annotations associated with the requested node
	 * Generates the response page.	*/
	private void showAnnotation(WikiDataRequest request, String NodeID) {
		try {
			
			HtmlPage page = new HtmlPage();
			DatabaseNode node= request.node();
						
			page.add("<table id=\"AnnotationTable\">");
			page.add("<tr>");
			page.add("<td>");
			page.add("<a class=\"close_annotation\" href=\"#\" onClick=\"CloseAnnotation()\"><img border=0 align=\"right\" src=\"/pictures/close_annotation.gif\"></a>");
			page.add("</tr>");
			page.add("</td>");
						
			if(node.hasAnnotation())
			{	
				AnnotationList annotations = request.node().annotation();
				
				for (int i=0; i<annotations.size();i++)
				{
					int a_id=annotations.get(i).id();
					page.add("<tr id=\""+NodeID +"/" + a_id + "\">");
					page.add("<td>");
					page.add("<div id=\""+ NodeID +"_row_" + a_id + "\" class=\"SingleAnnotation\" align=\"left\"");
					
					page.add("<div class=\"DeleteAnnotation\">");
					page.add("<input id=\"DeleteAnnotationButton\" type=\"image\" src=\"/pictures/close_annotation.gif\" alt=\"Submit\" align=\"right\" onClick=\"DeleteAnnotation(\'"+ NodeID +"/" + a_id+ "\')\">"); 
					page.add("</div>");
					page.add("<br>");
					page.add("<div class=Annotation><b>" + annotations.get(i).user().fullName() + ": </b>" + annotations.get(i).text());
					page.add("</div>");
					page.add("<br>");
					page.add("<div class=AnnotationDate align=\"right\">" + annotations.get(i).date());
					page.add("</div>");
				...
				page.add("<tr>");
				page.add("<td>");
				page.add("<div class=\"addAnnotation\">");
				page.add("<textarea id=\"annotationText\" class=\"InputText\" align=\"left\"></textarea>");
				page.add("<br>");
				page.add("<div id=\"submitAnnotationButton\" style=\"display: block;\">");
				page.add("<input id=\"AnnotationButton\" type=\"button\" value=\"Comment\" align=\"right\" onClick=\"AddAnnotation(\'"+ NodeID + "\')\">");
				page.add("</div>");
				page.add("</div>");
				page.add("</td>");
				page.add("</tr>");
	 ...
	 * @throws SQLException 
	 */
	private void respondToDataRequest(WikiDataRequest request) throws java.io.IOException, org.dbwiki.exception.WikiException, SQLException {
		HtmlPage page = null;
		...
		if (request.type().isAction()) {
			...
			if (action.actionAnnotate()) {
				...
				if (request.parameters().hasParameter(ParameterDatabaseID)) {
					...
					System.out.println("in dbwiki-in actionupdate for node");
		...
		if (page == null) {
			...
			if ((isGetRequest) || (request.type().isCopy())) {// This is the default case where no action has been performed and no special content is requested
				...
				System.out.println("request.type().isTimemachineChanges() ??");
