...
public class RDBMSDatabase implements Database, DatabaseConstants {
	...
	public QueryResultSet query(String query) throws org.dbwiki.exception.WikiException {
	 ...
	 /* NEW FOR TESTING --DELETE and also remove importing logger */
	public QueryResultSet query(String query, Logger log) throws org.dbwiki.exception.WikiException {

		log.info("The query is: "+query);
		return QueryStatement.createStatement(this,query,log).execute();
	}
	...
	/** Takes a DatabaseSchema and user id and connection
	 * Stores the schema in the database schema table
	 * Updates the Database's schema to be the provided schema, reloading from database to get timestamp information 
	 * @param con
	 ...
	 */
	public void initializeSchema(Connection con, DatabaseSchema databaseSchema, User user) throws org.dbwiki.exception.WikiException {
		
		try {
			assert(_schema.root() == null); // a sneaky way of checking whether schema has been initialized
			_connector.storeSchema( con, _wiki.name(), databaseSchema, user, _versionIndex );
