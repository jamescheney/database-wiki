...
//import java.util.logging.Logger; //new-for logging purpose
...
public abstract class DatabaseConnector implements DatabaseConstants, WikiServerConstants {
	...
	/**
	 * New-for logging purpose
	 * @param log
	 * @return
	 * @throws org.dbwiki.exception.WikiException
	 */
	/**
	public Connection getConnection(Logger log) throws org.dbwiki.exception.WikiException {
		log.info("entered get connection in DatabaseConnector");
		try {
			log.info("The getConnection returned: " +DriverManager.getConnection(_url, _user, _password));
			return DriverManager.getConnection(_url, _user, _password);	
		} catch (java.sql.SQLException sqlException) {
			log.info("sql Exception in getConnection" +sqlException);
			throw new WikiFatalException(sqlException);
		}
	}
	*/
	...
	public void dropDatabase(Connection con, String dbName) throws org.dbwiki.exception.WikiException {
		try {
			...
			dropView(stmt,dbName + MySQLDatabaseConnector.viewSchemaIndexBase); //added for the drop  of dbname_veindex_base view
	...
	public void storeSchema(Connection con, String dbName, DatabaseSchema schema, User user, SQLVersionIndex versionIndex) throws java.sql.SQLException, WikiException {
		Statement statement = con.createStatement();	
