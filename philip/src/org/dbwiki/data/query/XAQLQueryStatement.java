...
public class XAQLQueryStatement extends QueryStatement {
	...
	/* NEW FOR TESTING--DELETE and remove import logger */
	public XAQLQueryStatement(Database database, String queryExpression, Logger log) throws org.dbwiki.exception.WikiException {
		
		_database = database;
		log.info("_database is: "+_database);
		XAQLSyntaxParser parser = Parboiled.createParser(XAQLSyntaxParser.class);
		...
		if (result.hasErrors()) {
    		...
    		//result.parseTreeRoot.getValue().print("", "\t");
        	_query = new QueryStatementGenerator().getStatement(database, result.parseTreeRoot.getValue());
        	log.info("new querystatement generator");
        }
	}
