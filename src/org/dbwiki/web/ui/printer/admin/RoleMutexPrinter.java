...
public class RoleMutexPrinter extends HtmlContentPrinter{
	private String _headline;
	private DatabaseWiki _wiki;
	RolePolicy _rolePolicy;
	
	public RoleMutexPrinter(String headline, DatabaseWiki wiki) {
		_headline = headline;
		_wiki = wiki;
	...
	public void print(HtmlLinePrinter printer) throws WikiException {
		...
		if(conflictRolePairs != null) {
			for(Pair<Integer, Integer> pair : conflictRolePairs) {	
				...
				printer.addAlterBUTTON("submit", "action", RequestParameterAction.DeleteMutex, "<img src=\"/pictures/button_ok.gif\">", "Are you sure you want to remove the relation permanently?");
				printer.closeFORM();
				printer.closeTD();
		...
		printer.addHIDDEN(WikiServer.ParameterName, _wiki.name());
		printer.addRealBUTTON("submit", "action", RequestParameterAction.AddMutex, "<img src=\"/pictures/button_ok.gif\">");
