...
public class DatabaseWikiEntryAuthorizationPrinter extends HtmlContentPrinter {
	...
	public void print(HtmlLinePrinter printer) throws WikiException {
		...
		for(Integer i:new TreeSet<Integer>(_entryListing.keySet())){
			...
			if(flag){
				...
				printer.addRADIOBUTTON("Yes", entry_id+WikiServer.propertyReadPermission, "HoldPermission", (_policyListing.get(policyKey).get(entry_id).isRead() == DatabaseWikiProperties.HoldPermission));
				printer.addBR();
				printer.addRADIOBUTTON("No", entry_id+WikiServer.propertyReadPermission, "NoPermission", (_policyListing.get(policyKey).get(entry_id).isRead() != DatabaseWikiProperties.HoldPermission));
				printer.closeTD();
				//insert permission
				printer.openTD(CSS.CSSFormControl);
				printer.addRADIOBUTTON("Yes", entry_id+WikiServer.propertyInsertPermission, "HoldPermission", (_policyListing.get(policyKey).get(entry_id).isInsert() == DatabaseWikiProperties.HoldPermission));
				printer.addBR();
				printer.addRADIOBUTTON("No", entry_id+WikiServer.propertyInsertPermission, "NoPermission", (_policyListing.get(policyKey).get(entry_id).isInsert() != DatabaseWikiProperties.HoldPermission));
				printer.closeTD();
				//delete permission
				printer.openTD(CSS.CSSFormControl);
				printer.addRADIOBUTTON("Yes", entry_id+WikiServer.propertyDeletePermission, "HoldPermission", (_policyListing.get(policyKey).get(entry_id).isDelete() == DatabaseWikiProperties.HoldPermission));
				printer.addBR();
				printer.addRADIOBUTTON("No", entry_id+WikiServer.propertyDeletePermission, "NoPermission", (_policyListing.get(policyKey).get(entry_id).isDelete() != DatabaseWikiProperties.HoldPermission));
				printer.closeTD();
				//update permission
				printer.openTD(CSS.CSSFormControl);
				printer.addRADIOBUTTON("Yes", entry_id+WikiServer.propertyUpdatePermission, "HoldPermission", (_policyListing.get(policyKey).get(entry_id).isUpdate() == DatabaseWikiProperties.HoldPermission));
				printer.addBR();
				printer.addRADIOBUTTON("No", entry_id+WikiServer.propertyUpdatePermission, "NoPermission", (_policyListing.get(policyKey).get(entry_id).isUpdate() != DatabaseWikiProperties.HoldPermission));
				printer.closeTD();
			}else{
				//read permission
				printer.openTD(CSS.CSSFormControl);
