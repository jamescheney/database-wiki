...
public class VersionClauseGenerator {
	...
	public VersionClause getVersionClause(VersionIndex versionIndex, XAQLToken token) throws org.dbwiki.exception.WikiException {
		...
		if (token.type() != XAQLToken.VERSION_CLAUSE && token.type() != XAQLToken.COMPARING_VERSION_CLAUSE) {
		...
		boolean isComparing = token.type() == XAQLToken.COMPARING_VERSION_CLAUSE;
		
		if (!timestamp.equals("") && !timestamp.equals(TimeSequence.OpenIntervalChar)) {
			return new VersionClause(new TimeSequence(timestamp), isComparing);
		} else {
			return new VersionClause(new TimeSequence(Integer.MAX_VALUE), isComparing);
		}
	}
}
