...
public class QueryResultSet implements QueryNodeHandler {
	...
	public Vector<DatabaseNode> getNodes()
	{
		 return _nodes;
	}
	
   public  void Reconstructtoversion()
   {
	  int DatabaseNodeValueSize = ((DatabaseGroupNode)this.get(0)).children().size();
  	      for(int i =1; i<DatabaseNodeValueSize; i++)
  	      {
  	    	 for (int j =0; j<DatabaseNodeValueSize;j++){
  	    		((DatabaseAttributeNode)this.get(0)).value().SerVersionValue(i);
//  	    		DatabaseGroupNode VersionDatabaseAttributeNode =(DatabaseGroupNode)this.get(0);
//  	    		VersionDatabaseAttributeNode.value().SerVersionValue(i);
  	    		DatabaseAttributeNode NewVersionNode =	(DatabaseAttributeNode)this.get(0);
  	    	  _nodes.add(NewVersionNode);
  	    	 }
  	      }
   }
}
