...
/* jcheney: Seems not to be used
 * 
 */

@Deprecated
public class WikiServerAuthenticator extends Authenticator {
	/*
	 * Public Constants
	 */
	...
	public WikiServerAuthenticator(String realm, int mode, WikiServer server, UserListing users) {
		_mode = mode;
		_realm = realm;
		_server = server;
		_users = users;
	...
	public synchronized Result authenticate(HttpExchange exchange) {
		...
		// required. That is, the last path component has a '.'
		...
		// Test whether the request is for the server or one of the
		// Database Wikis. Based on the outcome the authenticationMode
		// is set.
		int authenticationMode = _mode;
		if (path.length() > 0) {
			pos = path.indexOf('/', 1);
			DatabaseWiki wiki = null;
			if (pos != -1) {
				wiki = _server.get(path.substring(1, pos));
			} else {
				wiki = _server.get(path.substring(1));
			}
			if (wiki != null) {
				authenticationMode = wiki.authenticator().getAuthenticationMode();
			}
		}
		...
		if (auth == null) {
			if ((authenticationMode == AuthenticateAlways)
					|| ((authenticationMode == AuthenticateWriteOnly) && (isProtectedRequest))
			...
			if ((authenticationMode == AuthenticateAlways) || ((authenticationMode == AuthenticateWriteOnly) && (isProtectedRequest))) {
