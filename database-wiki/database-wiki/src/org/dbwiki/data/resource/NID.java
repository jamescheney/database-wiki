...
public class NID implements ResourceIdentifier {
	/*
	 * Private Variables
	 */
	...
	public NID(int nodeID) {
		_nodeID = nodeID;
	}
	
	public NID(RequestURL url) throws org.dbwiki.exception.WikiException {
		if (url.size() == 0) {
			_nodeID = -1;
		} else if (url.size() == 1) {
			try {
	...
	public boolean equals(ResourceIdentifier identifier) {
		return (this.nodeID() == ((NID)identifier).nodeID());
	}
