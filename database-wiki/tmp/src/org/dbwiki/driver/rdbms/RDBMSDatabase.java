...
import org.dbwiki.data.query.QueryStatementFactory;
...
public class RDBMSDatabase implements Database, DatabaseConstants {
	 ...
	 * Constructors
	 */
	public RDBMSDatabase(DatabaseWiki wiki, JDBCConnector connector) throws org.dbwiki.exception.WikiException {
		...
		try {
			...
			_versionIndex = new SQLVersionIndex(con, wiki.name(), wiki.users());
			_schema  = new SQLDatabaseSchema(con, wiki.name());
			con.close();
	...
	public synchronized void activate(ResourceIdentifier identifier, User user) throws org.dbwiki.exception.WikiException {
		...
		DatabaseNode node = new DatabaseReader().get(con, this, (NID)identifier);
	...
	public synchronized void annotate(ResourceIdentifier identifier, Annotation annotation) throws org.dbwiki.exception.WikiException {
		Connection con = _connector.getConnection();
		new DatabaseWriter(con, this).insertAnnotation((NID)identifier, annotation);
		try {
			con.close();
		} catch (java.sql.SQLException sqlException) {
			throw new WikiFatalException(sqlException);
	...
	public RDBMSDatabaseListing content() throws org.dbwiki.exception.WikiException {
		return new DatabaseContentReader().get(_connector.getConnection(), this);
	}
	...
	public ResourceIdentifier getIdentifierForParameterString(String parameterValue) throws org.dbwiki.exception.WikiException {
		try {
			return new NID(Integer.parseInt(parameterValue));
		} catch (java.lang.NumberFormatException exception) {
			throw new WikiRequestException(WikiRequestException.InvalidUrl, parameterValue);
		}
	}

	public ResourceIdentifier getIdentifierForURL(RequestURL url) throws org.dbwiki.exception.WikiException {
		return new NID(url);
	}
		...
		try {
			...
			try {
				if (identifier.isRootIdentifier()) {
					...
					nodeIdentifier = new DatabaseWriter(con, this).insertNode((NID)identifier, node, version);
				}
				_versionIndex.add(version);
				_versionIndex.store(con);
			} catch (org.dbwiki.exception.WikiException wikiException) {
	...
	public synchronized void insertEntity(GroupEntity parent, String name, byte type, User user) throws org.dbwiki.exception.WikiException {
		if (!org.dbwiki.data.schema.Schema.isValidName(name)) {
		...
		if (type == EntityTypeAttribute) {
			...
			entity = new AttributeEntityImpl(_schema.size(), name, parent);
		} else if (type == EntityTypeGroup) {
			entity = new GroupEntityImpl(_schema.size(), name, parent);
		} else {
			throw new WikiSchemaException(WikiSchemaException.InvalidEntityType, String.valueOf(type));
		}
		
		Connection con = _connector.getConnection();
		new DatabaseWriter(con, this).insertEntity(entity, user);
		try {
			con.close();
		} catch (java.sql.SQLException sqlException) {
			throw new WikiFatalException(sqlException);
	...
	public synchronized void paste(ResourceIdentifier target, PasteNode pasteNode, String sourceURL, User user) throws org.dbwiki.exception.WikiException {
		...
		if (pasteNode.isElement()) {
			...
			Connection con = _connector.getConnection();			
			try {
				SQLDatabaseSchema schema = new SQLDatabaseSchema(con, _wiki.name());
					...
					try {
						...
						try {
							if (target.isRootIdentifier()) {
								...
								new DatabaseWriter(con, this).insertNode((NID)target, insertNode, version);
							}
							_versionIndex.add(version);
							_versionIndex.store(con);
							for (int iEntity = schema().size(); iEntity < schema.size(); iEntity++) {
								new DatabaseWriter(con, this).insertEntity(schema.get(iEntity), user);
							}
						} catch (org.dbwiki.exception.WikiException wikiException) {
							con.rollback();
							con.close();
	...
	public QueryResultSet query(String query) throws org.dbwiki.exception.WikiException {
		QueryStatement qStmt = new QueryStatementFactory(this).createStatement(query);
		if (qStmt.isNIDStatement()) {
			return new VectorQueryResultSet(get(new NID(((NIDQueryStatement)qStmt).nodeID())));
		} else if (qStmt.isWikiPathStatement()) {
			try {
				Connection con = _connector.getConnection();
				QueryResultSet rs = new QueryEvaluator().evaluate(con, this, (WikiPathQueryStatement)qStmt);
				con.close();
				return rs;
			} catch (java.sql.SQLException sqlException) {
				throw new WikiFatalException(sqlException);
	...
	public DatabaseContent search(String query) throws org.dbwiki.exception.WikiException {
		...
		if (keywords.size() > 0) {
			...
			try {
				...
				while (rs.next()) {
					NID identifier = new NID(rs.getInt(RelDataColEntry));
	...
	private DocumentNode getPasteInsertNode(GroupEntity parentEntity, PasteElementNode sourceNode, DatabaseSchema schema) throws org.dbwiki.exception.WikiException {
		...
		if (parentEntity != null) {
			...
			if (documentEntity == null) {
				if (_wiki.getAutoSchemaChanges() == DatabaseWiki.AutoSchemaChangesAllow) {
					if (sourceNode.isAttribute()) {
						documentEntity = new AttributeEntityImpl(schema.size(), sourceNode.label(), parentEntity);
					} else {
						documentEntity = new GroupEntityImpl(schema.size(), sourceNode.label(), parentEntity);
					}
			...
			if (_wiki.getAutoSchemaChanges() == DatabaseWiki.AutoSchemaChangesAllow) {
				documentEntity = new GroupEntityImpl(schema.size(), sourceNode.label(), null);
				schema.add(documentEntity);
			} else if (_wiki.getAutoSchemaChanges() == DatabaseWiki.AutoSchemaChangesNever) {
	...
	private void updateTimestamp(Connection con, DatabaseNode node, Timestamp timestamp) throws org.dbwiki.exception.WikiException {
		TimeSequenceInterval interval = ((TimeSequence)timestamp).lastInterval();
		
		NID nid = (NID)node.identifier();
		if (node.hasTimestamp() && !interval.isOpen()) {
			new DatabaseWriter(con, this).updateTimestamp(nid, interval);
		} else {
			new DatabaseWriter(con, this).insertTimestamp(nid, interval);
		}
		node.setTimestamp(timestamp);
	}
	
	private void insertTimestamp(Connection con, DatabaseNode node, Timestamp timestamp) throws org.dbwiki.exception.WikiException {
		TimeSequenceInterval interval = ((TimeSequence)timestamp).lastInterval();
