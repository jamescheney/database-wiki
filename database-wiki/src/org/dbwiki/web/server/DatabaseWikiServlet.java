...
public class DatabaseWikiServlet extends DatabaseWiki {
	...
	public void handle(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(_authenticator.authenticate(request)) {
				...
				if (pos != -1) {
					_server.sendFile(filename, response);
				} else {
					if (_server.serverLog() != null) {
						_server.serverLog().logRequest(request);
					}
					RequestURL url = new RequestURL(new ServletExchangeWrapper(
							request, response), _database.identifier().linkPrefix());
			...
			try {
				HtmlServletSender.send(HtmlTemplateDecorator.decorate(_template,
						new DatabaseWikiContentGenerator(this.identifier(),
								this.getTitle(), this.cssLinePrinter(),
								wikiException)), response);
	...
	private void respondToDataRequest(WikiDataRequest request, 
		...
		if (request.type().isAction()) {
			...
			response.setHeader("Set-Cookie", HttpRequest.CookiePropertyCopyBuffer + "=" + URLEncoder.encode(sourceURL, "UTF-8") + "; path=/; ");
			...
			this.respondToExportJSONRequest(request,
					new ExportJSONNodeWriter(), response);
			return;
		}
		...
		if (page == null) {
			...
			if ((isGetRequest) || (request.type().isCopy())) {// This is the
				...
				throw new WikiRequestException(
						WikiRequestException.InvalidRequest, httpRequest.getRequestURL().toString());
			}
			page = HtmlTemplateDecorator.decorate(_template, contentGenerator);
		}

		// Send the resulting page to the user.
		HtmlServletSender.send(page, response);
	}
	 ...
	 * @throws org.dbwiki.exception.WikiException
	 */
	private void respondToPageRequest(WikiPageRequest request, HttpServletRequest httpRequest,
			HttpServletResponse response) throws java.io.IOException,
			org.dbwiki.exception.WikiException {
		...
		if (request.type().isDelete()) {
			...
			HtmlServletSender.send(new RedirectPage(request.wri().databaseIdentifier()
					.databaseHomepage()), response);
			return;
	...
	private void respondToSchemaRequest(WikiSchemaRequest request,
		...
		if (request.type().isDelete()) {
			...
			if (request.schema().parent() != null) {
				HtmlServletSender.send(new RedirectPage(request, request.schema()
						.parent().identifier()), response);
				return;
			} else {
				isIndexRequest = true;
			}
	...
	private void respondToExportXMLRequest(WikiDataRequest request,
			NodeWriter writer, HttpServletResponse response)
			throws org.dbwiki.exception.WikiException {
		...
		try {
			...
			if (request.isRootRequest()) {
				...
				out.close();
				_server.sendXML(response, new FileInputStream(tmpFile));
				tmpFile.delete();
			} else {
				...
				database().export(request.wri().resourceIdentifier(),
						versionNumber, writer);
				out.close();
				_server.sendXML(response, new ByteArrayInputStream(buf
		...
		try {
			...
			if (request.isRootRequest()) {
				...
				database().export(request.wri().resourceIdentifier(),
						versionNumber, writer);
				out.close();
				_server.sendJSON(response, new FileInputStream(tmpFile));
				tmpFile.delete();
			} else {
				StringWriter buf = new StringWriter();
				BufferedWriter out = new BufferedWriter(buf);
				...
				database().export(request.wri().resourceIdentifier(),
						versionNumber, writer);
				out.close();
				_server.sendJSON(response, new ByteArrayInputStream(buf
						.toString().getBytes("UTF-8")));
			}
		} catch (java.io.IOException ioException) {
			throw new WikiFatalException(ioException);
