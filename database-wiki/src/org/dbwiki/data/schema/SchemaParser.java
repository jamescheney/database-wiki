...
public class SchemaParser {
	...
	private GroupEntity parseElement(String text, GroupEntity parent, DatabaseSchema schema) throws org.dbwiki.exception.WikiException {
		...
		GroupEntity entity = new ImportGroupEntityImpl(schema.size(), elementName, parent);
		if (!org.dbwiki.data.schema.Schema.isValidName(entity.label())) {
	...
	private void parseElementBody(String text, GroupEntity parent, DatabaseSchema schema) throws org.dbwiki.exception.WikiException {
		...
		while (elementBody.length() > 0) {
			if (elementBody.startsWith(attributeIndicator)) {
				...
				AttributeEntity attribute = new ImportAttributeEntityImpl(schema.size(), attributeName, parent);
				if (!org.dbwiki.data.schema.Schema.isValidName(attribute.label())) {
	...
	private GroupEntity parseEntry(String text, DatabaseSchema schema) throws org.dbwiki.exception.WikiException {
		// $name {...}
		...
		GroupEntity entity = new ImportGroupEntityImpl(schema.size(), entryName, null);
		if (!org.dbwiki.data.schema.Schema.isValidName(entity.label())) {
