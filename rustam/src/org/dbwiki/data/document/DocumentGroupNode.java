...
public class DocumentGroupNode extends DocumentNode {
    ...
    public void doDeweyNumbering (String init) 
    {
    	int number = 1;
    	if (Dewey.isEmpty()) {
    		Dewey = (String)init;
    		//System.out.println(this.label()+" whose Dewey is "+this.Dewey);
    	}
    	for (int i=0; i<children().size();i++) {
    		DocumentNode child = children().get(i);
    		child.Dewey = Dewey+"."+number;
    		//System.out.println(child.label()+" whose Dewey is "+child.Dewey);
    		number++;
    		
    		if (child.isGroup()) {
    			DocumentGroupNode grandChild =  (DocumentGroupNode) child;
    			grandChild.doDeweyNumbering(Dewey);
    		}
