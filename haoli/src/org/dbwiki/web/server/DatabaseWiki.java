...
import org.dbwiki.web.ui.printer.ObjectMapPrinter;
...
import org.dbwiki.web.ui.printer.visual.VisualMenuPrinter;
import org.dbwiki.web.ui.printer.visual.VisualNodePrinter;
import org.dbwiki.web.ui.printer.visual.VisualPathPrinter;
...
public class DatabaseWiki implements HttpHandler, Comparable<DatabaseWiki> {
	...
	public static final String VisualRequestPrefix = "visual";
	...
	public void handle(HttpExchange exchange) throws java.io.IOException {
		try {
			...
			if (pos != -1) {
				...
				} else if (url.isVisualRequest()) {
					respondToVisualRequest(new WikiVisualRequest(this, url), url);
				}
			}
	...
	private void respondToDataRequest(WikiDataRequest request) throws java.io.IOException, org.dbwiki.exception.WikiException {
		...
		if (page == null) {
			...
			if ((isGetRequest) || (request.type().isCopy())) {// This is the default case where no action has been performed and no special content is requested
				...
				//add map
				contentGenerator.put(DatabaseWikiContentGenerator.ContentMap, new ObjectMapPrinter(request, _layouter));
...
//		}
	...
	/** Respond to request for chart visualisation
	 * Like respondToDataRequest, this first checks the operation is a GET or POST.
	 * If POST, then the action is performed and the response redirects.
	 * If GET, then an appropriate ContentGenerator is constructed and 
	 * used to render the schema node.
	 ...
	 * @throws org.dbwiki.exception.WikiException
	 */
	private void respondToVisualRequest(WikiVisualRequest request, RequestURL url) throws java.io.IOException, org.dbwiki.exception.WikiException {
		// TODO: fill in and tidy up
		// 
		// Most of the code here is copied and pasted from above.
...
//		
		...
		if (isGetRequest) {
			contentGenerator.put(DatabaseWikiContentGenerator.ContentTimemachine, new TimemachinePrinter(request));
			contentGenerator.put(DatabaseWikiContentGenerator.ContentMenu, new VisualMenuPrinter(request));
			contentGenerator.put(DatabaseWikiContentGenerator.ContentObjectLink, new VisualPathPrinter(request, _layouter));
			contentGenerator.put(DatabaseWikiContentGenerator.ContentContent, new VisualNodePrinter(request, _layouter));
		} else if ((isIndexRequest) || (request.type().isDelete())) { // || (action.actionInsert())) {
			contentGenerator.put(DatabaseWikiContentGenerator.ContentTimemachine, new TimemachinePrinter(request));
			contentGenerator.put(DatabaseWikiContentGenerator.ContentMenu, new VisualMenuPrinter(request));
			contentGenerator.put(DatabaseWikiContentGenerator.ContentContent, new VisualNodePrinter(request, _layouter));
