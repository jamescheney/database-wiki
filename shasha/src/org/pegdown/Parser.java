...
public class Parser extends BaseParser<Node> implements SimpleNodeTypes, Extensions {
    ...
    Rule Inline() {
        return FirstOf(new ArrayBuilder<Rule>()
                .add(Link(), Str(), Endline(), UlOrStarLine(), Space(), Strong(), Emph(), Query(), Image(), Code(), InlineHtml(),
                        Entity(), EscapedChar())
    ...
    // FIXME: Add path parsing and parse queries to QueryStatement instead of strings.
    Rule Query() {
    	return Sequence(
    			'?',
    			OneOrMore(TestNot('?'), ANY),
    			//Path("?"),
    	    	push(new QueryNode(match())),
    	    	'?' 
    	);
    }
    ...
    @MemoMismatches
    Rule Link() {
        return FirstOf(WikiLink(),ExplicitLink(), ReferenceLink(), AutoLinkUrl(), AutoLinkEmail());
    }
    
    Rule WikiLink() {
    	return Sequence(
    			"[[",
    			OneOrMore(TestNot(" "), TestNot("]]"), ANY),
