...
public class DataUpdateFormPrinter implements HtmlContentPrinter {
	...
	public void print(HtmlLinePrinter body) throws org.dbwiki.exception.WikiException {
		if (_request.parameters().hasParameter(RequestParameter.ParameterCreate)) {
			...
			if (node.isElement()) {
				...
				//System.out.println("parent"+element.parent().parent());
	...
	public String getTextareaLine(String name, String value, int height) {
		return "<textarea name=\"" + RequestParameter.TextFieldIndicator +  name + "\" id=\"text1\""+" style=\"width: 99%; height: " + height + "px\">" + value + "</textarea>";
	}
	...
	public void printInsertLine(SchemaNode schema, HtmlLinePrinter body) {
		SchemaLayout layout = _layouter.get(schema);
		//System.out.println("parent:"+schema.parent());
		if(schema.parent().toString().startsWith("ANNOTATION")){
			body.openTR();
			body.addcanvas2("");
			body.closeTR();
			body.addBR();
		...
		body.closeTD();
		if(schema.parent().toString().startsWith("ANNOTATION")){
		body.text("<input type=\"button\" name=\"bt2\" value=\"Update\" onclick=\"returnSquare()\"><br>");
		}
		body.closeTR();
	}
	...
	public void printUpdateLine(DatabaseAttributeNode attribute, HtmlLinePrinter body) {
		...
		if (value != null) {
			...
			if(locationList!=null){
                ...
                String function="return drawRegion('"+annotation+locationList+"',2)";
                body.text("<input type=\"button\" name=\"bt1\" value=\"Show\" onclick=\""+function+"\">");
                body.text("<input type=\"button\" name=\"bt2\" value=\"Update\" onclick=\"returnSquare()\"><br>");
			}
			body.closeTD();
			body.closeTR();
