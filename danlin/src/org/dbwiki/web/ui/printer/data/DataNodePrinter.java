...
import org.dbwiki.data.database.DatabaseImage;
...
public class DataNodePrinter implements HtmlContentPrinter {
        ...
        public void printGroupNode(DatabaseGroupNode node, RequestParameterVersion versionParameter, HtmlLinePrinter body) throws org.dbwiki.exception.WikiException {
                ...
                String type;
                type=".jpg";
                DatabaseImage imageInfo = node.findIMG(type);
                _imageName = imageInfo.getImageName();
                //ArrayList<String> locationList=node.findLocation("monkey");
                //
                System.out.println("2aass");
                ...
                for (int i = 0; i < children.size(); i++) {
                        ...
                        if (lines.size()>0) {
                                ...
                                //
                                if(i==imageInfo.getImageLine())
                                {
                                        //Paint paint = new Paint(imageInfo.getImageName());
                                        //paint.mains();
                                        body.addcanvas1(_imageName);
                                        body.add(lines);
                                        //body.addANNImg(imageInfo.getImageName());
                                        //body.addJS();
                                        //body.addTEXTAREA("annotations", "90", value)
        ...
        public void printTextNode(DatabaseTextNode node, HtmlLinePrinter body) {
                ...
                //System.out.println("node"+node);
                String locationList=node.findLocation();
                ...
                if (locationList!=null){
                        body.addcanvas1("");
                        //body.addHIDDEN("location", locationList);
                }
                if (active) {
                        body.openTD(layout.getCSS(CSS.CSSObjectValueActive));
                } else {
                ...
                System.out.println("3aass");

                //if (node.value().startsWith("SHOW_"))
                //{
                if(locationList!=null){
                        //body.linkWithOnClick(target, onclick, text, cssClass);
        ...
        private String getNodeLink(DatabaseNode node, RequestParameterVersion versionParameter) {
                ...
                //System.out.println(node.identifier().toURLString());
                return target;
        }
        ...
        private boolean printAttributeValue(DatabaseAttributeNode attribute, RequestParameterVersion versionParameter, SchemaLayout layout, HtmlLinePrinter content) throws org.dbwiki.exception.WikiException {
                ...
                for (int iValue = 0; iValue < attribute.value().size(); iValue++) {
                        ...
                        String locationList=value.findLocation();
                        ...
                        if (versionParameter.matches(value)) {
                                ...
                                if (value.getTimestamp().isCurrent()) {
                                        if(locationList!=null){
                                            String annotation=value.findAnnotation(locationList);
                                            String function="return drawRegion('"+annotation+locationList+"',this)";
                                                content.linkWithOnClick(target, function,  annotation, layout.getCSS(CSS.CSSContentValueActive));
                                        ...
                                        if(locationList!=null){
                                            String annotation=value.findAnnotation(locationList);
                                            String function="return drawRegion('"+annotation+locationList+"')";
                                                content.linkWithOnClick(target, function, annotation, layout.getCSS(CSS.CSSContentValueInactive));
                                        }
                                        else{
        ...
        private boolean printNodesInListStyle(SchemaNodeList list, RequestParameterVersion versionParameter, SchemaLayout layout, HtmlLinePrinter content) throws org.dbwiki.exception.WikiException {
                ...
                for (int iNode = 0; iNode < list.size(); iNode++) {
                        ...
                        if (versionParameter.matches(element)) {//System.out.println(element);
                                boolean active = element.getTimestamp().isCurrent();
                                //String locationList=element.findLocation();
                                //String function="drawSqure("+locationList+",this)";
                                //System.out.println("if find locationlist"+locationList);
                                content.openTR();
                                ...
                                if (layout.getLabelAlignment().isTopAlign()) {
                                        ...
                                        if (active) {
                                                content.openTD(layout.getCSS(CSS.CSSContentLeftLabelActive));
                                                //if(locationList!=null){
                                                //      content.linkWithOnClick(target, function, layout.getLabel(element, versionParameter), layout.getCSS(CSS.CSSContentLabelActive));
                                                //}
                                                //else{
                                                        content.link(target, layout.getLabel(element, versionParameter), layout.getCSS(CSS.CSSContentLabelActive));
                                                //}                                            
